...
public class Img implements Serializable {
    ...
    public static Img from(File imageFile) {
        return new Img(ImgUtils.read(imageFile));
    }
    ...
    public static Img from(InputStream in) {
        return new Img(ImgUtils.read(in));
    }
    ...
    public static Img from(ImageInputStream imageStream) {
        return new Img(ImgUtils.read(imageStream));
    }
    ...
    public static Img from(URL imageUrl) {
        return new Img(ImgUtils.read(imageUrl));
    }
    ...
    public static Img from(Image image) {
        return new Img(ImgUtils.toBufferedImage(image));
    }
    ...
    public Img(BufferedImage srcImage, String targetImageType) {
        ...
        if (null == targetImageType) {
            if (srcImage.getType() == BufferedImage.TYPE_INT_ARGB
                ...
                targetImageType = ImgUtils.IMAGE_TYPE_PNG;
            } else {
                targetImageType = ImgUtils.IMAGE_TYPE_JPG;
            }
        }
     ...
     * @param imgType 图片格式
     * @return this
     * @see ImgUtils#IMAGE_TYPE_JPG
     * @see ImgUtils#IMAGE_TYPE_PNG
     */
    ...
    public Img scale(float scale) {
        ...
        // PNG图片特殊处理
        if (ImgUtils.IMAGE_TYPE_PNG.equals(this.targetImageType)) {
            // 修正float转double导致的精度丢失
            final double scaleDouble = NumberUtils.toDouble(scale);
            this.targetImage = ImgUtils.transform(AffineTransform.getScaleInstance(scaleDouble, scaleDouble),
                    ImgUtils.toBufferedImage(srcImg, this.targetImageType));
        } else {
            // 缩放后的图片宽
            String srcImgWithStr = Integer.toString(srcImg.getWidth(null));
            BigDecimal widthBigDecimal = new BigDecimal(srcImgWithStr);
            BigDecimal scaleBigDecimal = new BigDecimal(Float.toString(scale));
            final int width = widthBigDecimal.multiply(scaleBigDecimal).intValue();
            // 缩放后的图片高
            BigDecimal srcImgBigDecimal = new BigDecimal(srcImgWithStr);
            final int height = srcImgBigDecimal.multiply(scaleBigDecimal).intValue();
            scale(width, height);
        }
        return this;
    }
    ...
    public Img scale(int width, int height, int scaleType) {
        ...
        if (ImgUtils.IMAGE_TYPE_PNG.equals(this.targetImageType)) {
            ...
            // 宽度缩放比
            BigDecimal widthBigDecimal = new BigDecimal(Float.toString(width));
            BigDecimal srcWidthBigDecimal = new BigDecimal(Float.toString(srcWidth));
            final double sx = widthBigDecimal.divide(srcWidthBigDecimal, 10, RoundingMode.HALF_UP).doubleValue();
            // 高度缩放比
            BigDecimal heightBigDecimal = new BigDecimal(Float.toString(height));
            BigDecimal srcHeightBigDecimal = new BigDecimal(Float.toString(srcHeight));
            final double sy = heightBigDecimal.divide(srcHeightBigDecimal, 10, RoundingMode.HALF_UP).doubleValue();
            this.targetImage = ImgUtils.transform(AffineTransform.getScaleInstance(sx, sy),
                    ImgUtils.toBufferedImage(srcImg, this.targetImageType));
        } else {
    ...
    public Img scale(int width, int height, Color fixedColor) {
        ...
        BigDecimal heightBigDecimal = new BigDecimal(Integer.toString(height));
        BigDecimal srcHeightBigDecimal = new BigDecimal(Integer.toString(srcHeight));
        double heightRatio = heightBigDecimal.divide(srcHeightBigDecimal, 10, RoundingMode.HALF_UP).doubleValue();
        BigDecimal widthBigDecimal = new BigDecimal(Integer.toString(width));
        double widthRatio = widthBigDecimal.divide(srcHeightBigDecimal, 10, RoundingMode.HALF_UP).doubleValue();
    ...
    public Img cut(Rectangle rectangle) {
        ...
        this.targetImage = ImgUtils.filter(cropFilter, srcImage);
        return this;
    }
    ...
    public Img round(double arc) {
        ...
        // 通过弧度占比计算弧度
        BigDecimal arcBigDecimal = new BigDecimal(Double.toString(arc));
        BigDecimal minBigDecimal = new BigDecimal(Float.toString(Math.min(width, height)));
        arc = arcBigDecimal.multiply(minBigDecimal).doubleValue();
     ...
     */
    public Img gray() {
        this.targetImage = ImgUtils.colorConvert(ColorSpace.getInstance(ColorSpace.CS_GRAY), getValidSrcBufferedImg());
        return this;
    }
     ...
     */
    public Img binary() {
        this.targetImage = ImgUtils.copyImage(getValidSrcImg(), BufferedImage.TYPE_BYTE_BINARY);
        return this;
    }
    ...
    public Img pressText(String pressText, Color color, Font font, Point point, float alpha) {
        final BufferedImage targetImage = ImgUtils.toBufferedImage(getValidSrcImg(), this.targetImageType);

        if (null == font) {
            // 默认字体
            font = new Font(Font.SANS_SERIF, Font.PLAIN, (int) (targetImage.getHeight() * 0.75));
        }
    ...
    public Img pressTextFull(String pressText, Color color, Font font, int lineHeight, int degree, float alpha) {
        ...
        Dimension dimension;
        try {
            final int width = fontMetrics.stringWidth(pressText);
            final int height = fontMetrics.getAscent() - fontMetrics.getLeading() - fontMetrics.getDescent();
    ...
    public Img pressImage(Image pressImg, Rectangle rectangle, float alpha) {
        ...
        this.targetImage = draw(ImgUtils.toBufferedImage(targetImg, this.targetImageType), pressImg, rectangle, alpha);
        return this;
    }
    ...
    public Img stroke(Color color, Stroke stroke) {
        final BufferedImage image = ImgUtils.toBufferedImage(getValidSrcImg(), this.targetImageType);
        int width = image.getWidth(null);
        int height = image.getHeight(null);
        Graphics2D g = image.createGraphics();
    ...
    public boolean write(OutputStream out) {
        return write(ImgUtils.getImageOutputStream(out));
    }
    ...
    public boolean write(ImageOutputStream targetImageStream) {
        Assert.isTrue(StringUtils.isNotBlank(this.targetImageType), "Target image type is blank !");
        ...
        return ImgUtils.write(targetImage, this.targetImageType, targetImageStream, this.quality, this.backgroundColor);
    }
    ...
    public boolean write(File targetFile) {
        final String formatName = FileNameUtils.extName(targetFile);
        ...
        try {
            out = ImgUtils.getImageOutputStream(targetFile);
            return write(out);
        } finally {
    ...
    private Image getValidSrcImg() {
        return this.targetImage == null ? this.srcImage : this.targetImage;
    }
    ...
    private BufferedImage getValidSrcBufferedImg() {
        return ImgUtils.toBufferedImage(getValidSrcImg(), this.targetImageType);
    }
    ...
    private Rectangle fixRectangle(Rectangle rectangle, int baseWidth, int baseHeight) {
        if (this.positionBaseCentre) {
            final Point pointBaseCentre = ImgUtils.getPointBaseCentre(rectangle, baseWidth, baseHeight);
            // 修正图片位置从背景的中心计算
