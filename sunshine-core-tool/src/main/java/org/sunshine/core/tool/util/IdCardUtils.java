...
public class IdCardUtils {
    ...
    public static String convert15To18(String idCard) {
        ...
        if (RegexUtils.match(PatternPool.NUMBERS, idCard)) {
            // 获取出生年月日
            String birthday = idCard.substring(6, 12);
            int sYear = DateUtils.parse(birthday, "yyMMdd", temporal -> temporal.get(ChronoField.YEAR));
            // 获取出生年(完全表现形式,如：2010)
            if (sYear > 2000) {
                // 2000年之后不存在15位身份证号，此处用于修复此问题的判断
                sYear -= 100;
            }
            idCard18 = StringUtils.builder().append(idCard, 0, 6).append(sYear).append(idCard.substring(8));
            // 获取校验位
    ...
    public static String convert18To15(String idCard) {
        if (StringUtils.isNotBlank(idCard) && IdCardUtils.isValidCard18(idCard)) {
    ...
    public static boolean isValidCard18(String idCard, boolean ignoreCase) {
        ...
        if (RegexUtils.match(PatternPool.NUMBERS, code17)) {
            // 获取校验位
    ...
    public static String getBirth(String idCard) {
        Assert.isTrue(StringUtils.isNotBlank(idCard), "id card must be not blank!");
    ...
    public static Date getBirthDate(String idCard) {
        ...
        return null == birthByIdCard ? null : DateUtils.parse(birthByIdCard, "yyyyMMdd");
    }
    ...
    public static int getAgeByIdCard(String idCard) {
        return getAgeByIdCard(idCard, DateUtils.date());
    }
    ...
    public static class IdCard implements Serializable {
        ...
        public IdCard(String idCard) {
            ...
            this.birthDate = IdCardUtils.getBirthDate(idCard);
