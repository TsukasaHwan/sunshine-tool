...
public class PartMask implements MaskStrategy {
    ...
    private static final Map<Integer, String> MASK_CODES = new HashMap<>();

    static {
        //预生成指定长度的掩码星号
        for (int i = 1; i <= 10; i++) {
            MASK_CODES.put(i, getMaskCodeByNum(i));
        }
    }
    ...
    private static String getMaskCodeByNum(int num) {
        String maskCode = MASK_CODES.get(num);
        if (maskCode != null) {
            return maskCode;
        }
    ...
    public String mask(String source, int[] params) {
        ...
        return source.substring(0, leftKeep).concat(getMaskCodeByNum(source.length() - leftKeep - rightKeep))
                .concat(source.substring(source.length() - rightKeep));
    }
}
